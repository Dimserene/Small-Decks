...
SMODS.Back{
	...
	loc_txt = {
		...
		text ={
			"Start with a Deck",
			"full of {C:red,T:tag_foil}foil{} cards"
		},
    },
	apply = function()
		G.E_MANAGER:add_event(Event({
			func = function()
				for i = #G.playing_cards, 1, -1 do
					G.playing_cards[i]:set_ability(G.P_CENTERS.tag_foil)
					G.playing_cards[i]:set_edition({
						negative = true
					}, true, true)
				end
...
SMODS.Back{
	...
	apply = function()
		G.E_MANAGER:add_event(Event({
			func = function()
				for i = #G.playing_cards, 1, -1 do
					G.playing_cards[i]:set_ability(G.P_CENTERS.tag_Holographic)
					G.playing_cards[i]:set_edition({
						negative = true
					}, true, true)
				end
...
SMODS.Back{
	...
	apply = function()
		G.E_MANAGER:add_event(Event({
			func = function()
				for i = #G.playing_cards, 1, -1 do
					G.playing_cards[i]:set_ability(G.P_CENTERS.tag_polychrome)
					G.playing_cards[i]:set_edition({
						negative = true
					}, true, true)
				end
...
SMODS.Back{
	...
	apply = function()
		G.E_MANAGER:add_event(Event({
			func = function()
				for i = #G.playing_cards, 1, -1 do
					G.playing_cards[i]:set_ability(G.P_CENTERS.tag_negative)
					G.playing_cards[i]:set_edition({
						negative = true
					}, true, true)
				end
